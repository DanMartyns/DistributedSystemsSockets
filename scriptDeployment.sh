...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws01.ua.pt << EOF
	cd Logger/
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws02.ua.pt << EOF
	cd BettingCenter/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws03.ua.pt << EOF
	cd ControlCenter/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws04.ua.pt << EOF
	cd Paddock/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws05.ua.pt << EOF
	cd RacingTrack/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws06.ua.pt << EOF
	cd Stable/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws07.ua.pt << EOF
	cd Broker/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws08.ua.pt << EOF
	cd Horses/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws09.ua.pt << EOF
	cd Spectators/
	find . -name "*.java" > files.txt
	javac @files.txt
	rm files.txt
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws01.ua.pt << EOF
	...
	nohup java -cp /home/sd0401/Logger/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar BettingCenter na máquina ${bold}2${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws02.ua.pt << EOF
	cd BettingCenter/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/BettingCenter/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar ControlCenter na máquina ${bold}3${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws03.ua.pt << EOF
	cd ControlCenter/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/ControlCenter/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar Paddock na máquina ${bold}4${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws04.ua.pt << EOF
	cd Paddock/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/Paddock/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar RacingTrack na máquina ${bold}5${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws05.ua.pt << EOF
	cd RacingTrack/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/RacingTrack/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar Stable na máquina ${bold}6${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws06.ua.pt << EOF
	cd Stable/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/Stable/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF
...
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws07.ua.pt << EOF
	...
	nohup java -cp /home/sd0401/Broker/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar Horses na máquina ${bold}8${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws08.ua.pt << EOF
	cd Horses/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/Horses/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF

echo -e "\n${bold}->${normal} A executar Spectators na máquina ${bold}9${normal}"
sshpass -e ssh -o StrictHostKeyChecking=no sd0401@l040101-ws09.ua.pt << EOF
	cd Spectators/src/main/java/MainPackage/
	nohup java -cp /home/sd0401/Spectators/src/main/java MainPackage.MainProgram > /dev/null 2>&1 &
EOF
